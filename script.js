...
const fotoNumaralari = rastgeleFotoNumaralari();
function rastgeleFotoNumaralari() {
    let numaralar = [];
  
    while (numaralar.length < 8) {
      // 1-99 arasında rastgele bir sayı seçin
      let sayi = Math.floor(Math.random() * 99) + 1;
      
      // Çift sayılar için kontrol yapın
      if (sayi % 2 === 0) {
